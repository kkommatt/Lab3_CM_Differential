...
def compute_and_plot():
    ...
    euler_result_label = Label(inputs_frame,
                               text=f"Euler's Method: {euler_y_vals[-1]:.4f}, Error: {np.abs(euler_y_vals[-1] - exact_solution[-1]):.10f}")
    euler_result_label.grid(row=8, column=0, columnspan=2)

    rk_result_label = Label(inputs_frame,
                            text=f"Runge-Kutta Method: {rk_y_vals[-1]:.4f}, Error: {np.abs(rk_y_vals[-1] - exact_solution[-1]):.10f}")
